...
"dd if=/dev/zero of=/var/sys-vmem.swap bs=1024 count=1048576"
...
"docker login -u=ohrsan -p=$password >> /home/ec2-user/instance-creation.log 2>&1"
...
new_instance_id=$($AWS --output json ec2 run-instances $INSTANCE_DRY_RUN --image-id  $INSTANCE_AMI_ID \
                       ...
                       --block-device-mappings "[{\"DeviceName\":\"/dev/xvda\",\"Ebs\":{\"VolumeSize\":12,\"DeleteOnTermination\":true}}]"\
